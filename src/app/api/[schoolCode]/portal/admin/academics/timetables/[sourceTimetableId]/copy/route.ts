...
export async function POST(
  request: Request,
  { params: routeParams }: { params: { schoolCode: string; sourceTimetableId: string } }
) {
  const { schoolCode, sourceTimetableId } = routeParams;
  ...
  try {
    ...
    const { name, academicYearId, classId, termId, description } = body;
    ...
    const validPeriodsToCopy = sourceTimetable.periods
      ...
      .map((p: any) => ({ // Use any for p here if lean() makes subdocument fields strings
        ...
        location: p.location,
        _id: new mongoose.Types.ObjectId(), // Generate new _id for the subdocument
      }));
